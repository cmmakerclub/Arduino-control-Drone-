...
float getcompasscourse() {
  ...
  ayf = AccY / 57.0; //Convert to rad

  axf = AccX / 57.0; //Convert to rad

  xh = c_magnetom_x * cos(ayf) + c_magnetom_y * sin(ayf) * sin(axf) - c_magnetom_z * cos(axf) * sin(ayf);

  yh = c_magnetom_y * cos(axf) + c_magnetom_z * sin(axf);
